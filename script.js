...
window.addEventListener('DOMContentLoaded', function(){
  // '변환'번튼을 누르면 result에 원 텍스트를 변형해 넣어준다.
  document.querySelector('.change').addEventListener('click', function(){
    var changedText = translator(document.querySelector('.original-text').value); // translator 함수
    document.querySelector('.result-text').innerText = changedText; //  변형 된 문장을 결과에 넣어준다.
  });
});
